...
import type { Faculty, UpdateFacultyPayload } from "../../types/api.types";

interface EditFacultyModalProps {
  isOpen: boolean;
  onClose: () => void;
  faculty: Faculty;
  onSubmit: (data: UpdateFacultyPayload) => Promise<any>;
}
...
export default function EditFacultyModal({
  ...
  const [formData, setFormData] = useState<UpdateFacultyPayload>({
    name: faculty.name,
  ...
  return (
    <Modal isOpen={isOpen} onClose={handleClose} title="Edit Faculty">
